...
const User = sequelize.define(
  'User',
  {
    ...
    firstName: { // String that's not allowed to be empty
    ...
    surName: { // String that's not allowed to be empty
      type: DataTypes.STRING,
      allowNull: false,
      validate: {
        notEmpty: true,
    ...
    userName: { // String that must be unique or must not have a constant with the same value
      type: DataTypes.STRING,
    ...
    email: {  // String that must be unique or must not have a constant with the same value
      type: DataTypes.STRING,
      allowNull: false,
      unique: true,
      validate: {
        isEmail: true,
        isGmail(value) { // Must end with one of the constant values in order for it to be accepted as an email
          const allowedDomains = ['@gmail.com', '@yahoo.com', '@hotmail.com', '@outlook.com'];
          if (!allowedDomains.some(domain => value.endsWith(domain))) {
            throw new Error('Please provide a valid Gmail, Yahoo, or Outlook email address.');
          }
        },
      },
    },
    phoneNumber: {
      ...
      unique: true,
      validate: {
        isMacedonianNumber(value) { // Removes spaces and non numeric values from the phone number except for the '+'
          let cleanedValue = value.replace(/[\s()\-]/g, '').replace(/[^0-9+]/g, '');
          if (cleanedValue.startsWith('07')) { // Must start with '07' or '+389 7'
            cleanedValue = `+389${cleanedValue.slice(1)}`;
          } const macedonianRegex = /^\+3897\d{7}$/;
          if (!macedonianRegex.test(cleanedValue)) {
            throw new Error('Please provide a valid Macedonian phone number (e.g., +389 70 707 070 or 070 707 070).');
