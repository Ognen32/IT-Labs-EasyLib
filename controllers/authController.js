...
import { forgotPasswordService, resetPasswordService, registerUserService, loginUserService, logoutUserService } from '../services/authService.js';

// Used for handling up the services's API request & response logic

export const registerUser = catchAsyncError(async (req, res, next) => {
    const { userName, email, phoneNumber, firstName, surName, password, role, dateOfBirth, city, address } = req.body;
    await registerUserService(req, res); // Calls the service to handle the registry
  });

  export const loginUser = catchAsyncError(async (req, res, next) => {
    const { email, password } = req.body;
    if (!email || !password) { // checks if both are entered
